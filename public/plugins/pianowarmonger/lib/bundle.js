...
/***/ function(module, exports, __webpack_require__) {
	...
	const SONG_MAPPING = {
	  'Test': testSong,
	  "Oda a la Alegria": odeToJoy,
	  "El Leon duerme ya": LionSleepsTonight,
	  "Los hobbits": ConcerningHobbits
	};
	...
	const populateSong = function(songName) {
	  ...
	  let longNotes;
	  switch(songName) {
	    case 'Oda a la Alegria':
	      longNotes = [12, 14, 27, 29];
	      break;
	    case 'El Leon duerme ya':
	      longNotes = [14, 29];
	      break;
	    case 'Los hobbits':
...
/***/ function(module, exports, __webpack_require__) {
	...
	GameView.prototype.endGame = function() {
	  ...
	  if (this.game.score < (this.game.numNotes/2)) {
	    message = `Parece que necesitas mas practica. Acertaste ${fraction} notas.`;
	  } else if (this.game.score < (3*(this.game.numNotes)/4)) {
	    message = `Nada mal! acertaste ${fraction} notas.`;
	  } else if (this.game.score < (this.game.numNotes)){
	    message = `Bien hecho! acertaste ${fraction} notas. ¡Continua con el buen trabajo!`;
	  } else {
	    message = `Felicitaciones! Tienes el puntaje perfecto, no tengo nada mas que enseñarte`;
	  }
